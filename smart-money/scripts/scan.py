...
def normalize_deal(raw: dict, deal_type: str, watch_set: set[str]) -> dict | None:
    try:
...
def build_symbol_summary(deals: list[dict]) -> list[dict]:
    ...
    summary.sort(key=lambda r: abs(r["net_value_cr"]), reverse=True)
    return summary
...
def main():
    ...
    for option_type, label in (("bulk_deals", "bulk"), ("block_deals", "block")):
        ...
        for r in raw:
            norm = normalize_deal(r, label, watch_set)
            if norm:
    ...
    log.info("Wrote %s (%d deals, %d insider filings)", args.out, len(all_deals_raw), len(insider_filings))
